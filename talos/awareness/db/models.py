...
class Artifact(Base):
    """Large-file metadata (C15, Phase 8): the bytes live on the local
    filesystem under the configured data directory; paths are generated by
    the store — callers (and the LLM) can never choose them."""
    ...
    artifact_id: Mapped[UUID] = mapped_column(primary_key=True, server_default=sa.text("gen_random_uuid()"))
    kind: Mapped[str] = mapped_column(sa.String(100))
    display_name: Mapped[str] = mapped_column(sa.String(300))
    relative_path: Mapped[str] = mapped_column(sa.String(500), unique=True)
