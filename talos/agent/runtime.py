...
def _build_tool_definitions(
    mcp_client: Any, command: str | None = None, *, history_tool_names: set[str] | None = None
) -> list[dict[str, Any]]:
    if TOOLS_DISABLED:
    ...
    if command is not None and scope_tools not in {"0", "false", "no", "off"}:
        scoped = _scope_specialized_tools(tool_defs, command)
        scoped_names = {tool["name"] for tool in scoped}
        # Restore only current available definitions for historical calls. Never
        # use archived schemas to resurrect removed/disabled providers or tools.
        tool_defs = [
            tool for tool in tool_defs
            if tool["name"] in scoped_names or tool["name"] in (history_tool_names or set())
        ]
    ...
    *,
    ...
    with _get_conversation_lock(thread_key), _turn_in_flight():
        ...
        tool_defs = _build_tool_definitions(
            mcp_client, command, history_tool_names=history_tool_names
        )
        tool_build_ms = round((time.perf_counter() - tool_build_started) * 1000.0, 1)
