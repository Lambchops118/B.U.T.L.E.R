...
| OQ-D | Quad-pump fuse sensing has no viable ADC path. The fuse signal is an analog divider on GP6-GP9, but those pins are digital-only and the Pico W exposes ADC on GP26-GP28 only (three, not four). Which resolution does the owner want: (a) add an external I2C ADC such as an ADS1115, (b) measure the divider and confirm fuse-good > ~2.0 V / fuse-blown < ~0.8 V so it can be read as a debounced digital signal, or (c) leave fuse monitoring permanently out? Until then every channel reports `unknown` and no fuse interlock exists (ADR-019). | Before any fuse-fault alerting or fuse-based safety claim |
| OQ-E | Which physical pot does each of relay GP0, GP1, GP2, GP3 actually drive? The owner confirmed channel 1 = pot 1 and channel 2 = pot 2, and the firmware maps channel N → GP(N-1) in index order, but that ordering has not been verified against the wiring. Verifiable on the bench by energizing one channel at a time and observing which pump runs. | Before the first unattended watering on the new board |
