...
| OQ-K | Which sensitive debug feeds should the runtime explicitly publish: exact assembled prompts, tool names/arguments/results, generated-versus-audible output, and continuously sampled idle audio RMS/ambient baseline? What redaction, retention, rate, buffer bound, opt-in, and network-access policy applies to each? | Before adding any new hot-path or content-bearing telemetry producer; the initial dashboard truthfully shows these as unavailable. |
