...
| OQ-H | Which AEC path is proven on the deployed voice hardware: Windows communications-mode endpoint AEC with an explicit render reference, or a direct WebRTC APM/AEC3 integration? Exact input/output endpoint IDs, Windows build, driver-exposed effects, double-talk recordings, and maintenance/licensing evidence for any native binding are not yet confirmed. | Before implementing `docs/voice/BARGE_IN_REDESIGN_PLAN.md` Phase C |
