...
screen. New `butler/services/display_power.py` is driven from `sleep_mode._set`,
...
`butler/logs/llm_io_<UTC timestamp>_<pid>.jsonl` file while continuing to send
...
ReSpeaker barge-in and experimental idle VAD fail closed because Butler renders
...
| Configuration | Opt-in: `BUTLER_AWARENESS_BRIEFING_ENABLED=1`. Default schedule 08:00 host local, arrival enabled, cap 3, channel voice. Optional model ranking defaults off; enabling it requires configured `CHAT_MODEL` and loopback Ollama. No live settings were changed. |
...
| Files | Added `butler/awareness/context/briefing.py`, `butler/awareness/history/briefing.py`, two briefing test modules, and `SESSION_LOG.md` (2026-09-06 PHASE 09A entry); updated config, subsystem README, decisions, questions, and this status. |
...
| Decisions made | ADR-001..054 in `DECISIONS.md`; ADR-050..053 cover recording presence/interaction/agent outcomes without transcripts, `POST /ingest` running the identical pipeline rather than a bypass, transport pinning so internal sources cannot be forged over the LAN broker, and relevance ordering that can never reorder across priority bands (renumbered from this branch's original ADR-028..031, which collided with the plant-waterer ADR-028..031 below — see ADR-050 in `DECISIONS.md`); ADR-054 lets a source opt out of offline detection (renumbered from the original ADR-032); ADR-030 stages the plant-waterer network connect inside the watchdog budget; ADR-031 adds MQTT liveness detection and the radio/board reset escalation ladder; ADR-032 sets the Wi-Fi join deadline from measured join times; ADR-028 corrects the plant-waterer GPIO map from board netlist evidence and supersedes ADR-022; ADR-029 aligns the `water_plants` timeout with the 30 s firmware cycle; ADR-026 keeps the debug console standalone/read-only/bounded, and ADR-027 forbids substituting console-local hardware metrics for the remote Butler host. |
| Assumptions confirmed | `DISCOVERY.md` §12/§15; open-question table in `OPEN_QUESTIONS.md`; plant-waterer relay direction/polarity, concurrency and run-time limits, pot mapping, and fuse policy confirmed by the owner in chat 2026-07-26; the GPIO pin numbering from that session was wrong and is now taken from the `Controller_Board_mk2` netlist (ADR-028) |
...
| Tests last run | 2026-09-06: `.venv-awareness` unittest discovery over `tests/test_awareness_*.py` — **157 passed of 160**, against a baseline of 141/144 by the same command before this session, so the 16 new tests pass and nothing regressed. 2 errors are pre-existing and unrelated (`test_awareness_client_and_provider` cannot import `mcp` in that venv; it belongs to `.venv-main`), 1 skip is the test Mosquitto not running. `py_compile` passes on all touched modules, and every main-agent module changed here (`awareness_signals`, `jobs`, `router`, `agent/runtime`) imports cleanly in `.venv-main`, with `voice/agent` importing cleanly in `.venv-voice`. **Not run:** the main-venv suite — `.venv-main` lacks sqlalchemy/fastapi/mcp — so the router, jobs, runtime, voice, and MCP-provider edits are syntax-checked only and have not executed in a live agent process. No live voice session and no run against the production broker. Earlier, 2026-09-01: 79/79 `tests.test_quad_pump_firmware` passed (65 pre-existing plus 14 new `NetworkSupervisorTest` cases covering staged join, backoff, escalation, half-open detection, non-`OSError` client faults, and watchdog feeding); no existing test needed changing. `main.py`, `qp_net.py`, `qp_config.py` and `qp_controller.py` deployed to the Pico on COM6 and verified by on-device SHA-256 against the repository (`qp_ledger.py`/`qp_protocol.py` still differ by line endings only, as in the previous session). On-hardware verification: the board survived 45 s of the exact failure conditions (cold Wi-Fi join plus unreachable broker, one watchdog feed per iteration) with a worst single `ensure_connected()` of 3218 ms; the escalation ladder fired a radio reset at 5 failures; the real broker reconnected in 4513 ms afterwards. Live after deployment: `wifi_connected: true`, `mqtt_connected: true`, `rssi: -32`, `consecutive_failures: 0`, `radio_resets: 0`, all four relays off, heartbeats on schedule, time-to-online 4147 ms. No pump was activated. Earlier, 2026-08-28: 65/65 `tests.test_quad_pump_firmware` passed after the GPIO-map correction and two stale-assertion updates (`DEFAULT_RUN_SECONDS`, concurrency now queues rather than rejects); `actions.toml` re-parsed and verified. Firmware deployed to the Pico on COM6 and verified by on-device SHA-256 against the repository; board reports wifi/mqtt connected, rssi -41, all relays off, heartbeats flowing. A typo in the tracked `qp_secrets.py` `WIFI_SSID` (`Verizon_4LXXY` vs the actual `Verizon_4VLXXY`) was found and fixed during deployment; it had been preventing the board from joining the network at all. Earlier, 2026-08-09: 19 focused dashboard/existing-telemetry tests passed (including 8 dashboard data/HTTP/remote-metrics tests); targeted Python syntax compilation, JavaScript syntax check, local page/snapshot requests, and `git diff --check` passed. Awareness health tests were not run because the main interpreter lacks its separate SQLAlchemy dependency. Earlier the same day, 70 focused barge-in/VAD/STT/ASR-priority/acceptance/speaker/duplex tests passed. The prior 2026-07-27 live AEC evidence remains unchanged. |
...
| Files recently modified | New `butler/awareness/api/routes/ingest.py`, `butler/services/awareness_signals.py`, two test modules, and the 2026-09-06 handoff. Modified: awareness registry bootstrap/sources, ingestion pipeline/service, API app, context broker, alerts service, rules engine + `rules.toml` (policy version 1 → 2), reminders worker, config, README; main-agent `router.py`, `jobs.py`, `agent/runtime.py`, `voice/agent.py`, `mcp_servers/providers/awareness.py`; `settings.env`. Earlier: plant-waterer submodule `Firmware/main.py`, `Firmware/qp_net.py`, `Firmware/qp_config.py`, `Firmware/qp_controller.py`; `tests/test_quad_pump_firmware.py`; ADR-030/031/032 and the 2026-09-01 network-resilience handoff. Earlier still: new `butler/debug_dashboard/` service/static page, focused dashboard tests, README, ADR/open-question/status records, and the 2026-08-09 debug-dashboard handoff. Earlier voice settings/agent, bounded ASR queue, faster-whisper preload, and VAD files remain as recorded in the wake-latency handoff. |
| Next permitted task | Owner review of the human-context work, and authorization of a Phase 9 sub-phase. **Phase 9 (Proactive Briefing and Model-Selected Salience) is now specified** in `phases/PHASE_09_PROACTIVE_BRIEFING.md` and is NOT started: it separates the deterministic *moment* from model-selected *content*, and is split into 9A (assembler), 9B (triggers and delivery bookkeeping), 9C (model selection), 9D (feedback loop), each independently authorizable. Otherwise: Then, if approved, either (a) run the main-venv suite and a live agent/voice session to verify signal emission end to end, or (b) populate interaction `entity_ids` so conversation relevance does real work rather than scoring zero. Separately: commit the plant-waterer submodule pointer once its working-directory checkout matches the resolved merge (it now carries `5a96aa4` plus this session's changes) — still uncommitted; owner-observed relay-click verification on GP6-GP9, one channel at a time (moves water, so it was not run unattended); rotate the Wi-Fi/broker credentials exposed by the tracked `Firmware/qp_secrets.py` and untrack the file; owner review of the debug page and selection of any missing feed to instrument under OQ-K. Separately, the earlier owner-visible idle wake/pause/noise corpus and Phase F barge-in corpus/soak remain outstanding before voice rollout claims. |
| Required reading | `SESSION_LOG.md` (2026-09-06 HUMAN CONTEXT entry), the README "Human context" section, `butler/services/awareness_signals.py`, `butler/awareness/api/routes/ingest.py`, `butler/awareness/context/broker.py`, and ADR-050..053 in `DECISIONS.md`. Earlier: `SESSION_LOG.md` (2026-09-01 PLANT WATERER NETWORK RESILIENCE entry), ADR-030/031/032, `Peripherals/Pump-Power-Controller/Firmware/qp_net.py`; `SESSION_LOG.md` (2026-08-09 DEBUG DASHBOARD entry), the README Local Debug Dashboard section, `butler/debug_dashboard/server.py`, `butler/debug_dashboard/static/app.js`, ADR-026/ADR-027, and OQ-K/OQ-L. For voice follow-up, retain the wake-latency handoff and barge-in plan reading list. |
| Explicit stop condition | Human-context task is complete. **Do not begin unsorted/unmodeled-data handling** (observations table, promotion by repetition, salience decay) — it was explicitly excluded and needs its own owner decision. Do not add utterance-text capture, remote exposure of `/ingest`, or escalation rules for tool failures without authorization. Requested debug page is also complete; do not add prompt/tool/audio hot-path capture, remote exposure, launcher integration, or silent room recording without owner authorization. Prior stop conditions still hold: no prompt/tool/audio hot-path capture, no silent room recording, do not set `BUTLER_IDLE_VAD_CORPUS_ACCEPTED=1`, tune from guesses, or enable unaccepted barge-in. |
| Documentation follow-up (2026-07-16) | Added `like_im_a_child_or_golden_retriever.md`, a plain-language intern quick start covering immediate operation, Butler integration, maintenance, code paths, tests, safety invariants, limitations, and troubleshooting. Linked it from this documentation index. Validation: 96 awareness unit tests and 3 main-agent home-action tests pass; CLI help, relative-link targets, and `git diff --check` pass. Runtime, schemas, migrations, decisions, and open questions are unchanged. |
