...
| Decisions made | ADR-001..054 in `DECISIONS.md`; ADR-050..053 cover recording presence/interaction/agent outcomes without transcripts, `POST /ingest` running the identical pipeline rather than a bypass, transport pinning so internal sources cannot be forged over the LAN broker, and relevance ordering that can never reorder across priority bands (renumbered from this branch's original ADR-028..031, which collided with the quad-pump ADR-028..031 below — see ADR-050 in `DECISIONS.md`); ADR-054 lets a source opt out of offline detection (renumbered from the original ADR-032); ADR-030 stages the quad-pump network connect inside the watchdog budget; ADR-031 adds MQTT liveness detection and the radio/board reset escalation ladder; ADR-032 sets the Wi-Fi join deadline from measured join times; ADR-028 corrects the quad-pump GPIO map from board netlist evidence and supersedes ADR-022; ADR-029 aligns the `water_plants` timeout with the 30 s firmware cycle; ADR-026 keeps the debug console standalone/read-only/bounded, and ADR-027 forbids substituting console-local hardware metrics for the remote TALOS host. |
| Assumptions confirmed | `DISCOVERY.md` §12/§15; open-question table in `OPEN_QUESTIONS.md`; quad-pump relay direction/polarity, concurrency and run-time limits, pot mapping, and fuse policy confirmed by the owner in chat 2026-07-26; the GPIO pin numbering from that session was wrong and is now taken from the `Controller_Board_mk2` netlist (ADR-028) |
