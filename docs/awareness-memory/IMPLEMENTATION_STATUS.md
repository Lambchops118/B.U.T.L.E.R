...
| Tests last run | 2026-07-16: awareness venv — 97 tests (adds `tests.test_awareness_context_{unit,integration}`), all pass (integration requires `docker compose -f docker-compose.awareness.yml --profile test up -d --wait`); main venv — 14 tests (`tests.test_text_server_{phone_events,notify}`, `tests.test_awareness_client_and_provider`), all pass |
| Known failures | None |
...
| Next permitted task | Phase 6 — Long-term memory (`phases/PHASE_06_LONG_TERM_MEMORY.md`) |
| Required reading | Root `AGENTS.md`, this file, `DISCOVERY.md`, `phases/PHASE_06_LONG_TERM_MEMORY.md`, and its listed references |
