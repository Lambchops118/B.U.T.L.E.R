...
from butler.filesystem_diagnostics import resolve_allowed_root_paths as _resolve_allowed_root_paths
...
class _ServerConnection:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                "The MCP Python SDK is required for Butler MCP clients. "
...
class LocalMcpClient:
    def __init__(
        ...
        *,
        ...
        # An empty list is a legitimate state: the launcher can start Butler with
        ...
        self._bridge_timeout_seconds = (
            ...
            else float(os.getenv("BUTLER_MCP_BRIDGE_TIMEOUT", str(max(max_config_timeout + 5.0, 30.0))))
        )
        self._reconnect_attempts = (
            reconnect_attempts
            if reconnect_attempts is not None
            else max(0, int(os.getenv("BUTLER_MCP_RECONNECT_ATTEMPTS", "1")))
        )
        self._reconnect_backoff_seconds = (
            reconnect_backoff_seconds
            if reconnect_backoff_seconds is not None
            else max(0.0, float(os.getenv("BUTLER_MCP_RECONNECT_BACKOFF", "1.0")))
        )
        self._failure_threshold = (
            failure_threshold
            if failure_threshold is not None
            else max(1, int(os.getenv("BUTLER_MCP_FAILURE_THRESHOLD", "3")))
        )
    ...
    def start_server(self, server: str, *, background: bool = True) -> list[dict[str, Any]]:
        """Explicitly start (or warm) a named provider without restarting Butler.
            ...
            if not done:
                await self._mark_server_failed(
                    ...
                    TimeoutError(
                        ...
                        "Startup is still quarantined in the background; use retry_mcp_server after it finishes or restart Butler."
                    ),
                    cleanup=False,
                )
                return False
    ...
    def _filesystem_server_name() -> str:
        return (
            os.getenv("BUTLER_FILESYSTEM_SERVER_NAME", DEFAULT_FILESYSTEM_SERVER_NAME).strip()
...
def _apply_disabled_providers(configs: list[McpServerConfig]) -> None:
    ...
    default config is built) so it also covers a ``butler-local`` entry written
    out explicitly in ``BUTLER_MCP_SERVERS``.
    """

    disabled = sorted(_names_from_env("BUTLER_MCP_DISABLED_PROVIDERS"))
    if not disabled:
        return
    for config in configs:
        if "butler.mcp_server" in [str(arg) for arg in config.args]:
            config.args = [*config.args, "--disable-provider", ",".join(disabled)]


def _filesystem_writes_allowed() -> bool:
    raw = os.getenv("BUTLER_FILESYSTEM_ALLOW_WRITES", "").strip().lower()
...
def _optional_filesystem_server_config() -> McpServerConfig | None:
    roots = _resolve_allowed_root_paths(os.getenv("BUTLER_FILESYSTEM_ROOTS", ""))
    if not roots:
        return None

    command = os.getenv("BUTLER_FILESYSTEM_COMMAND", "npx").strip() or "npx"
    package = (
        os.getenv(
            "BUTLER_FILESYSTEM_PACKAGE",
    ...
    return McpServerConfig(
        name=os.getenv("BUTLER_FILESYSTEM_SERVER_NAME", DEFAULT_FILESYSTEM_SERVER_NAME).strip()
        or DEFAULT_FILESYSTEM_SERVER_NAME,
        transport="stdio",
        command=command,
        args=args,
        tool_prefix=os.getenv("BUTLER_FILESYSTEM_TOOL_PREFIX", DEFAULT_FILESYSTEM_TOOL_PREFIX).strip()
        or DEFAULT_FILESYSTEM_TOOL_PREFIX,
        timeout_seconds=float(os.getenv("BUTLER_FILESYSTEM_TIMEOUT", "120")),
        mode=os.getenv("BUTLER_FILESYSTEM_MODE", "").strip(),
...
def _optional_filesystem_diagnostics_server_config() -> McpServerConfig | None:
    ...
    env = {
        "BUTLER_FILESYSTEM_ROOTS": json.dumps([str(root) for root in roots]),
    }
    for key in (
        "BUTLER_FILESYSTEM_RG_TIMEOUT",
        "BUTLER_FILESYSTEM_MAX_TEXT_BYTES",
        "BUTLER_FILESYSTEM_DEFAULT_EXCLUDES",
    ):
    ...
    return McpServerConfig(
        name=(
            os.getenv(
                "BUTLER_FILESYSTEM_DIAGNOSTICS_SERVER_NAME",
        ...
        transport="stdio",
        command=sys.executable,
        args=["-m", "butler.mcp_filesystem_diagnostics_server"],
        env=env,
        tool_prefix=os.getenv("BUTLER_FILESYSTEM_TOOL_PREFIX", DEFAULT_FILESYSTEM_TOOL_PREFIX).strip()
        or DEFAULT_FILESYSTEM_TOOL_PREFIX,
        timeout_seconds=float(os.getenv("BUTLER_FILESYSTEM_DIAGNOSTICS_TIMEOUT", "120")),
        mode=os.getenv("BUTLER_FILESYSTEM_MODE", "").strip(),
...
def _optional_minecraft_server_configs() -> list[McpServerConfig]:
    ...
    return [
        ...
        McpServerConfig(
            ...
            args=["-m", "butler.mcp_minecraft_diagnostics_server"],
...
def _tools_globally_disabled() -> bool:
    """True when ``BUTLER_DISABLE_ALL_TOOLS`` turns the whole tool layer off.
    ...
    raw = os.getenv("BUTLER_DISABLE_ALL_TOOLS", "").strip().lower()
    return raw in {"1", "true", "yes", "on"}


def _load_mcp_server_configs() -> list[McpServerConfig]:
    """Configured MCP servers, minus any named in ``BUTLER_MCP_DISABLED_SERVERS``.
    ...
    disabled = _names_from_env("BUTLER_MCP_DISABLED_SERVERS")
    if disabled:
...
def _collect_mcp_server_configs() -> list[McpServerConfig]:
    raw_config = os.getenv("BUTLER_MCP_SERVERS", "").strip()
    if not raw_config:
    ...
    try:
        ...
        raise ValueError(
            "BUTLER_MCP_SERVERS must be valid JSON. "
    ...
    if not isinstance(parsed, list):
        raise ValueError("BUTLER_MCP_SERVERS must decode to a JSON array or object.")

    configs: list[McpServerConfig] = []
    seen_names: set[str] = set()
    for index, item in enumerate(parsed):
        if not isinstance(item, dict):
            raise ValueError(f"BUTLER_MCP_SERVERS entry {index} must be an object.")

        name = str(item.get("name") or "").strip()
        transport = str(item.get("transport") or "").strip()
        if not name:
            raise ValueError(f"BUTLER_MCP_SERVERS entry {index} is missing 'name'.")
        if not transport:
            raise ValueError(f"BUTLER_MCP_SERVERS entry {index} is missing 'transport'.")
        if name in seen_names:
            raise ValueError(f"BUTLER_MCP_SERVERS contains duplicate server name '{name}'.")
        seen_names.add(name)
        ...
        if not isinstance(args, list):
            raise ValueError(f"BUTLER_MCP_SERVERS entry '{name}' has non-list 'args'.")

        headers = item.get("headers", {})
        if headers is None:
            headers = {}
        if not isinstance(headers, dict):
            raise ValueError(f"BUTLER_MCP_SERVERS entry '{name}' has non-object 'headers'.")

        env = item.get("env", {})
        if env is None:
            env = {}
        if not isinstance(env, dict):
            raise ValueError(f"BUTLER_MCP_SERVERS entry '{name}' has non-object 'env'.")
