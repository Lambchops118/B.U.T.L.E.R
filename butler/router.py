...
from butler.config import env_bool
from butler.jobs import TERMINAL_STATUSES, JobManager, JobRecord, get_default_job_store
from butler.messages import AnnouncementPayload, Message, StatusPayload, TextPayload, VoicePayload
from butler.request_classifier import RequestClassification, classify_request
from butler.services import awareness_client, awareness_signals, sleep_mode
from butler.state_store import StateStore
...
VOICE_SPEAK_URL = os.getenv("BUTLER_VOICE_SPEAK_URL", "http://127.0.0.1:8610")
VOICE_SPEAK_ENABLED = env_bool("BUTLER_VOICE_SPEAK_ENABLED", True)
...
# rely on the local heuristic classifier. Set BUTLER_VOICE_MODEL_ROUTING=1 to
# restore the LLM route decision for voice commands.
VOICE_MODEL_ROUTING_ENABLED = env_bool("BUTLER_VOICE_MODEL_ROUTING", False)
...
def _classify_with_context(
    ...
    *,
    ...
    try:
        ...
        print(f"Butler route decision failed; falling back to heuristic classifier: {exc}")
