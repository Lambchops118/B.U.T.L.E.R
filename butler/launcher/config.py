"""Configuration for the Butler launcher.

Two kinds of state live here:

1. **Butler settings** already stored in ``settings.env`` (e.g. the Ollama model,
...
# butler/launcher/config.py -> repo root is two parents up.
...
_SETTINGS_DEFAULTS = {
    "BUTLER_LLM_MODEL": "hermes3:8b",
    "BUTLER_LLM_BASE_URL": "http://127.0.0.1:11434/v1",
    "BUTLER_LLM_THINK_MODE": "off",
...
class McpEntry:
    ...
    provider group inside the built-in ``butler-local`` aggregate server.
    """
...
# Default server names, mirroring the defaults in butler/mcp_client/client.py.
...
def mcp_catalog() -> list[McpEntry]:
    ...
    filesystem_ready = bool(get_active_setting("BUTLER_FILESYSTEM_ROOTS"))
    ...
    entries = [
        McpEntry(
            key="butler-local",
            label="butler-local (built-in tool server)",
            kind="server",
        ...
        McpEntry(
            key=_server_name("BUTLER_FILESYSTEM_SERVER_NAME"),
            label="Filesystem",
            kind="server",
            detail="needs BUTLER_FILESYSTEM_ROOTS",
            available=filesystem_ready,
        ),
        McpEntry(
            key=_server_name("BUTLER_FILESYSTEM_DIAGNOSTICS_SERVER_NAME"),
    ...
    for name in _extra_server_names():
        if name not in known:
            entries.append(
                McpEntry(key=name, label=name, kind="server", detail="from BUTLER_MCP_SERVERS")
            )
            known.add(name)
    return entries
...
def _extra_server_names() -> list[str]:
    """Server names declared explicitly in ``BUTLER_MCP_SERVERS``."""

    raw = get_active_setting("BUTLER_MCP_SERVERS")
    if not raw:
        return []
    try:
...
class LauncherConfig:
    ...
    # external client that reaches the main agent over BUTLER_TEXT_AGENT_URL, and
    ...
    # sibling of the Butler repo (see Supervisor._discord_root).
    ...
    # provider group, a new server in BUTLER_MCP_SERVERS) is on by default and an
    # old config file never silently hides it. The launcher turns these into
    # BUTLER_MCP_DISABLED_SERVERS / BUTLER_MCP_DISABLED_PROVIDERS for the main agent;
...
class Ports:
    ...
    def load(cls) -> "Ports":
        ...
        return cls(
            ollama=11434,
            awareness=as_int("BUTLER_AWARENESS_API_PORT", 8600),
            text_agent=as_int("TEXT_AGENT_PORT", 8420),
            awareness_host=get_setting("BUTLER_AWARENESS_API_HOST", "127.0.0.1"),
