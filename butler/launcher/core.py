"""Process supervision for the Butler stack.
...
def _venv_env(base: dict[str, str], python_path: Path) -> dict[str, str]:
    ...
    In particular the ``butler-local`` MCP server is configured with
...
def _api_model_env(base: dict[str, str], api_model: str) -> dict[str, str]:
    ...
    explicit ``BUTLER_LLM_API_KEY`` is set).
    """

    env = dict(base)
    env["BUTLER_LLM_BACKEND"] = "openai_chat"
    env["BUTLER_LLM_BASE_URL"] = "https://api.openai.com/v1"
    env["BUTLER_LLM_MODEL"] = api_model
    # Route STT to hosted whisper-1 instead of the local faster-whisper pass.
    env["BUTLER_LOCAL_STT"] = "0"
    env["BUTLER_REMOTE_STT_FALLBACK"] = "1"
    env["BUTLER_REMOTE_LLM_FALLBACK"] = "1"
    ...
    env["BUTLER_LLM_MAX_TOKENS_PARAM"] = "max_completion_tokens"
...
def _mcp_env(base: dict[str, str], cfg: LauncherConfig) -> dict[str, str]:
    ...
    env = dict(base)
    for name, keys in (
        ("BUTLER_MCP_DISABLED_SERVERS", cfg.disabled_mcp_servers),
        ("BUTLER_MCP_DISABLED_PROVIDERS", cfg.disabled_mcp_providers),
    ):
...
def _microphone_env(base: dict[str, str], cfg: LauncherConfig) -> dict[str, str]:
    ...
    env["BUTLER_MICROPHONE_PROFILE"] = name
    env["BUTLER_RECOGNIZER_ENERGY_THRESHOLD"] = profile.energy_threshold
    if not profile.windows_aec:
        ...
        # host (`python -m butler.voice.diagnostics.windows_aec_probe`). Both
        # deployed profiles have now passed; this stays as the fail-closed path
        # for any profile added later.
        env["BUTLER_BARGE_IN"] = "0"
        env["BUTLER_IDLE_VAD_ENDPOINTING"] = "0"
    return env
...
class Supervisor:
    """Starts, monitors, and stops the Butler component processes."""
    ...
    def _run_migrations(self) -> None:
        ...
        self._run_blocking(
            [str(py), "-m", "butler.awareness", "migrate"],
            name="migrate",
    ...
    def _start_awareness(self, base: dict[str, str]) -> None:
        ...
        self._spawn("awareness", [str(py), "-m", "butler.awareness", "serve"], env)
        self._wait_port(
            "awareness",
    ...
    def _start_main(self, base: dict[str, str]) -> None:
        ...
        env["BUTLER_LLM_DEBUG_STDOUT"] = "1"
        env["BUTLER_LLM_DEBUG_LOG_DIR"] = str(REPO_ROOT / "butler" / "logs")
        ...
        self._spawn("main", [str(py), "-m", "butler"], env)
        self._wait_port(
            "main",
    ...
    def _start_voice(self, base: dict[str, str]) -> None:
        ...
        # The voice worker reaches the main agent over BUTLER_TEXT_AGENT_URL. When
        # the launcher also starts the main agent locally, the worker must talk
        # to it on loopback. A real OS-level BUTLER_TEXT_AGENT_URL (e.g. a
        ...
        if self._cfg.start_main:
            ...
            env["BUTLER_TEXT_AGENT_URL"] = local_url
            self._say(f"pointing voice worker at local main agent: {local_url}")
        self._spawn("voice", [str(py), "-m", "butler.voice.worker"], env)
    ...
    def _discord_root(self) -> Path:
        ...
        sibling of the Butler repo. The Discord frontend is a separate repository,
    ...
    def _start_discord(self, base: dict[str, str]) -> None:
        ...
        plus the Rust voice gateway. Both run in the Discord repo, not Butler.

        The frontend talks to the main agent over ``BUTLER_TEXT_AGENT_URL`` (its
        ``LLM_PROVIDER=butler`` path), so when the launcher also starts the main
        ...
        # main agent when we started it; default it to the Butler brain backend.
        discord_env = dict(base)
        discord_env.setdefault("LLM_PROVIDER", "butler")
        if self._cfg.start_main:
            local_url = f"http://127.0.0.1:{self._ports.text_agent}"
            discord_env["BUTLER_TEXT_AGENT_URL"] = local_url
