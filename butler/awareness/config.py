...
All settings come from environment variables (``BUTLER_AWARENESS_*``) or the
repository ``.env`` file, matching the existing Butler convention. MQTT settings
fall back to the legacy ``MQTT_BROKER`` / ``MQTT_PORT`` names already used by
``butler/services/home_automation.py`` so one configuration drives both worlds.
...
# butler.config.load_environment(). Real environment variables still take
...
_ENV_ALIASES: dict[str, str] = {
    "mqtt_host": "BUTLER_AWARENESS_MQTT_HOST (or legacy MQTT_BROKER)",
    "mqtt_port": "BUTLER_AWARENESS_MQTT_PORT (or legacy MQTT_PORT)",
}
...
class AwarenessSettings(BaseSettings):
    ...
    # Live Postgres objects, deliberately still named talos_*: renaming them
    # would orphan the existing data volume.
    ...
    mqtt_host: str = Field(
        ...
        validation_alias=AliasChoices("BUTLER_AWARENESS_MQTT_HOST", "MQTT_BROKER"),
    )
    mqtt_port: int = Field(
        default=1883,
        validation_alias=AliasChoices("BUTLER_AWARENESS_MQTT_PORT", "MQTT_PORT"),
    )
    mqtt_tls: bool = False
    ...
    mqtt_client_id: str = "butler-awareness"
    ...
    timezone: str = Field(
        default="America/New_York", validation_alias=AliasChoices("BUTLER_TIMEZONE")
    )
    weather_location: str = Field(
        default="", validation_alias=AliasChoices("BUTLER_WEATHER_LOCATION")
    )
    weather_units: str = Field(
        default="imperial", validation_alias=AliasChoices("BUTLER_WEATHER_UNITS")
    )
    ...
    rules_path: Path | None = None  # default: butler/awareness/rules/rules.toml
...
def _env_name_for(field: str) -> str:
    ...
    upper = field.upper()
    if upper.startswith("BUTLER_AWARENESS_"):
        return upper
    return f"BUTLER_AWARENESS_{upper}"
