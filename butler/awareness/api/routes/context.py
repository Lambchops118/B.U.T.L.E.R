...
from butler.awareness.db.models import Alert, AlertEvent, Event
...
async def get_capabilities(request: Request) -> dict:
    ...
    if not db_ok:
        ...
        action_request = (
            "disabled: BUTLER_AWARENESS_API_TOKEN is not configured for "
    ...
    return {
        "capabilities": {
            ...
            "proactive_briefings": reads if request.app.state.settings.briefing_enabled else
                "disabled: BUTLER_AWARENESS_BRIEFING_ENABLED=0",
        }
    }
