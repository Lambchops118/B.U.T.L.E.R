...
backend for Butler. It runs as its **own process** (like the voice worker), on
...
# 3. Configuration: set BUTLER_AWARENESS_DB_PASSWORD in .env. To enable
#    physical action mutations, also set a shared BUTLER_AWARENESS_API_TOKEN
...
# 5. Verify, then serve.
.venv-awareness/bin/python -m butler.awareness check   # exit 0 = healthy
.venv-awareness/bin/python -m butler.awareness serve   # API on 127.0.0.1:8600
curl -s http://127.0.0.1:8600/health/components | python3 -m json.tool
```

## Migration policy

- Migrations live in `butler/awareness/db/migrations/versions/` and are applied
  **explicitly** via `python -m butler.awareness migrate` (or
  `alembic -c butler/awareness/alembic.ini upgrade head`) — never implicitly at
  process startup.
...
|---|---|---|
| `BUTLER_AWARENESS_DB_PASSWORD` | *(required)* | Postgres password (also used by docker compose) |
| `BUTLER_AWARENESS_DB_HOST` / `_DB_PORT` | `127.0.0.1` / `5433` | Postgres address |
| `BUTLER_AWARENESS_DB_NAME` / `_DB_USER` | `talos_awareness` / `butler` | Database / role |
| `BUTLER_AWARENESS_API_HOST` / `_API_PORT` | `127.0.0.1` / `8600` | Internal API bind |
| `BUTLER_AWARENESS_API_TOKEN` | *(unset; action mutations disabled)* | Shared bearer token required by action request/confirm/cancel routes and sent by the main-agent client |
| `BUTLER_AWARENESS_LOG_LEVEL` | `INFO` | Structured JSON log level |
| `BUTLER_AWARENESS_DATA_DIRECTORY` | `db/awareness` | Local artifact store root (used from Phase 8) |
| `BUTLER_AWARENESS_OLLAMA_HOST` | `http://127.0.0.1:11434` | Ollama (same machine per owner decision, still configurable) |
| `BUTLER_AWARENESS_CHAT_MODEL` / `_EMBEDDING_MODEL` | *(unset)* | Model names (used from Phase 5/6) |
| `BUTLER_AWARENESS_MQTT_HOST` / `_MQTT_PORT` | falls back to legacy `MQTT_BROKER` / `MQTT_PORT`, then `192.168.1.160:1883` | Existing Raspberry Pi Mosquitto broker (used from Phase 2) |
| `BUTLER_AWARENESS_MQTT_TLS` / `_MQTT_USERNAME` / `_MQTT_PASSWORD` / `_MQTT_CLIENT_ID` / … | see `config.py` | Broker security/session options |
| `BUTLER_AWARENESS_MAX_EVENT_PAYLOAD_BYTES` | `65536` | Ingestion payload bound |
| `BUTLER_AWARENESS_INGEST_API_ENABLED` | `1` | Accept internal/manual messages on `POST /ingest` (same pipeline, no bypass) |
| `BUTLER_AWARENESS_SIGNALS_ENABLED` | `1` | Read by the **main/voice** processes: emit presence, interaction, and agent-outcome signals |

## MQTT ingestion (Phase 2)

The `serve` process connects to the **existing** Raspberry Pi Mosquitto broker
(`BUTLER_AWARENESS_MQTT_HOST`, default `192.168.1.160:1883`; set
`BUTLER_AWARENESS_MQTT_ENABLED=0` to run API-only). No second production broker
...
`BUTLER_AWARENESS_FRESHNESS_INTERVAL_SECONDS`) marks state rows `stale` and
...
`BUTLER_AWARENESS_MAX_QUERY_RANGE_DAYS` / `_MAX_QUERY_POINTS` /
...
`BUTLER_AWARENESS_RULES_PATH`) validated by strict typed models
...
(`BUTLER_AWARENESS_QUIET_HOURS`) defer only noncritical availability — a
...
locks, dead-letters after `BUTLER_AWARENESS_OUTBOX_MAX_ATTEMPTS`, and supports
...
whenever `BUTLER_AWARENESS_NOTIFY_URL` is set and `BUTLER_AWARENESS_NOTIFY_VOICE_ENABLED`
...
recall without treating Butler's own words as new facts. Failed attempts and GUI/log
...
A deterministic `ReminderWorker` (interval `BUTLER_AWARENESS_REMINDER_INTERVAL_SECONDS`,
...
through the normal egress (default channel `BUTLER_AWARENESS_REMINDER_CHANNEL=voice`,
interruptibility `BUTLER_AWARENESS_REMINDER_INTERRUPTIBILITY=immediate`) and
...
`BUTLER_AWARENESS_SITUATION_TRANSITION_WINDOW_MINUTES`) → unhealthy sources.
...
`butler.services.awareness_client.snapshot_with_fallback(...)`: the rendered
situation when the backend answers within
`BUTLER_AWARENESS_CLIENT_TIMEOUT` (cached 5 s), else the legacy in-memory
snapshot — a backend outage degrades truthfully and never blocks commands.
Disable with `BUTLER_AWARENESS_SITUATION_ENABLED=0`.

### Read tools (MCP)

`butler/mcp_servers/providers/awareness.py` registers seven narrow read tools
...
Ollama (`BUTLER_AWARENESS_EMBEDDING_MODEL`, default `nomic-embed-text`,
...
one. Action mutations fail closed unless `BUTLER_AWARENESS_API_TOKEN` is
...
`python -m butler.awareness retention` prints the exact dry-run plan (per
policy: cutoff, eligible count, protections); `--execute` deletes in bounded
batches (`BUTLER_AWARENESS_RETENTION_BATCH_SIZE`), each batch its own
...
`python -m butler.awareness consolidate`: incident scopes with ≥3 active
...
`butler/awareness/artifacts.py`: bytes under `{data_directory}/artifacts/`
...
`python -m butler.awareness backup [--verify]` — pg_dump (custom format) runs
...
`BUTLER_AWARENESS_BACKUP_RETENTION_DAYS`. `--verify` restores into a scratch
...
pg_restore -U butler -d <db> --no-owner < dump`, `SELECT
timescaledb_post_restore()`, then `python -m butler.awareness check`.
...
- API binds loopback; **physical action** mutations are fail-closed (503
  until `BUTLER_AWARENESS_API_TOKEN` is set, then bearer-authenticated).
  ...
  The main agent sends it automatically when `BUTLER_AWARENESS_API_TOKEN`
...
`python -m butler.awareness.benchmark --events 2000`: **118 events/s**
...
Bearer-gated like every other mutation (`BUTLER_AWARENESS_API_TOKEN` when set),
and disabled entirely with `BUTLER_AWARENESS_INGEST_API_ENABLED=0`. The
pipeline is built at startup regardless of `BUTLER_AWARENESS_MQTT_ENABLED`, so
...
| `home/agent/talos/event` | `talos_agent` | `agent.job.*` / `agent.tool.failed` events on the `butler` agent entity |

Emission lives in [`butler/services/awareness_signals.py`](../services/awareness_signals.py)
...
`butler.awareness.context.briefing.BriefingAssembler(engine, settings).build(kind)`
...
`butler/awareness/history/briefing.py`. The `queries` audit records their time
...
| Setting (`BUTLER_AWARENESS_` prefix) | Default | Effect |
|---|---:|---|
