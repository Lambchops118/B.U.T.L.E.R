...
- ``voice``: authenticated ``POST /speak`` on the Butler text server, which
...
- ``gui``: authenticated ``POST /notify`` on the Butler text server, which
...
logger = get_logger("butler.awareness.notifications")
