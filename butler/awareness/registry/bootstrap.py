...
from butler.awareness.db.models import (
    Alert,
    AttentionItem,
    CurrentState,
    Entity,
...
_ENTITIES: list[dict[str, Any]] = [
    ...
    # entity_id stays "talos": it is a stored key, and the talos_agent source
    # and home/agent/talos/* topics both bind to it.
    {"entity_id": "talos", "display_name": "Butler agent", "entity_type": "agent", "location_id": "home"},
]
...
_SOURCES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "source_type": "agent",
        "display_name": "Butler main agent (internal signals)",
        "transport": "internal",
        "entity_id": "talos",
        "location_id": "home",
...
_ENTITY_MIGRATIONS: list[dict[str, Any]] = [
    ...
    {
        # 2026-09-10 rename. Only the label moves; entity_id stays "talos"
        # because the talos_agent source and home/agent/talos/* topics bind
        # to it.
        "entity_id": "talos",
        "column": "display_name",
        "expected": "TALOS agent",
        "value": "Butler agent",
    },
]
