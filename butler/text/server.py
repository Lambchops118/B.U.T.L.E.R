...
from butler.jobs import get_default_job_store
from butler.messages import AnnouncementPayload, EventPayload, Message, TextPayload
from butler.services import awareness_client, sleep_mode
...
class TextServerConfig:
    ...
    def from_env(cls) -> "TextServerConfig":
        ...
        phone_push_token = os.getenv("BUTLER_PHONE_PUSH_TOKEN", "").strip()
        return cls(
            enabled=enabled,
            host=host,
            port=port,
...
class TextAgentRequestHandler(BaseHTTPRequestHandler):
    ...
    server_version = "ButlerTextAgent/0.1"
    ...
    def _sleep_phrase_note(self, command: str, source: str) -> str | None:
        ...
        background job. What Butler *says* about it is still the model's own
    ...
    def _handle_phone_event(self) -> None:
        ...
        if not expected_token:
            self._write_json(
                HTTPStatus.UNAUTHORIZED,
                {"ok": False, "error": "Phone push ingress is not configured (BUTLER_PHONE_PUSH_TOKEN unset)."},
            )
            return
        ...
        try:
            from butler.phone import ingest_phone_bridge_snapshot
