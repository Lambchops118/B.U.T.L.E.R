...
def _base_url() -> str:
    return os.getenv("BUTLER_AWARENESS_API_URL", DEFAULT_BASE_URL).rstrip("/")


def _timeout() -> float:
    try:
        return float(os.getenv("BUTLER_AWARENESS_CLIENT_TIMEOUT", "1.5"))
    except ValueError:
        return 1.5


def _api_token() -> str:
    return os.getenv("BUTLER_AWARENESS_API_TOKEN", "").strip()


def situation_enabled() -> bool:
    return os.getenv("BUTLER_AWARENESS_SITUATION_ENABLED", "1").strip() not in {
        "0",
        "false",
        "no",
    }
...
def _auth_headers() -> dict[str, str]:
    token = os.getenv("BUTLER_AWARENESS_API_TOKEN", "").strip()
