...
def _broker() -> tuple[str, int]:
    ...
    host = os.getenv("BUTLER_DISPLAY_MQTT_HOST", "").strip() or os.getenv(
        "BUTLER_AWARENESS_MQTT_HOST", ""
    ).strip() or "192.168.1.160"
    return host, env_int("BUTLER_DISPLAY_MQTT_PORT", 1883)
...
def is_enabled() -> bool:
    ...
    """
    return env_bool("BUTLER_DISPLAY_POWER_ENABLED", True)
...
def _go_dark() -> None:
    from butler.services import tv_control
...
def apply(asleep: bool, *, block: bool = False) -> None:
    ...
    Called by :mod:`butler.services.sleep_mode` on every sleep/wake write, so
