"""Sleep mode: one shared night-mode flag for the whole Butler process tree.
...
- the physical display the panel is shown on is put into standby, and lit again
  on wake (:mod:`butler.services.display_power`). Sleep mode and a dark screen
...
from butler.config import REPO_ROOT, env_float, load_environment
...
STATE_PATH = resolve_state_path(os.getenv("BUTLER_SLEEP_STATE_PATH", ""))
...
# awake brightness on the glass. Turn it up with BUTLER_SLEEP_DIM_LEVEL (0.05 to
# 0.10 is a readable night-dark) if a true 1% is darker than you want.
DIM_LEVEL = min(max(env_float("BUTLER_SLEEP_DIM_LEVEL", 0.01), 0.0), 1.0)
...
# does not wait on an LLM to dim -- but the words Butler actually says are its
...
def _set(asleep: bool, reason: str) -> dict:
    ...
    new_state = {
        ...
        # put Butler to sleep while leaving the panel's intended level awake.
...
_WAKE_REQUEST_PATTERNS = tuple(
    re.compile(pattern)
    for pattern in (
        # "wake" only as an imperative aimed at Butler. Not "I will wake you in
...
# worker runs barge-in without requiring the wake word, so the tail of Butler's
...
LIBERAL_WAKE_GRACE_SECONDS = env_float("BUTLER_SLEEP_WAKE_GRACE_SECONDS", 8.0)
