...
from butler.messages import Message, VoicePayload
from butler.services import display_power, sleep_mode
...
def wake_display():
    ...
    drives that through ``butler.services.display_power``. This job stays as the
    backstop for the morning briefing, which clears sleep mode on its own when
    it is delivered (see butler/text/server.py) but may be disabled, deferred,
