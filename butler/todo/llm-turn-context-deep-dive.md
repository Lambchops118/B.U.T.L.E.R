# Butler LLM Turn Context Deep Dive

Date: 2026-07-24

## Executive summary

Butler has two distinct context and memory systems:
...
- `butler/agent/runtime.py::classify_request_route`
- `butler/router.py::_classify_with_context`
- `butler/router.py::_runtime_context_for_session`

## Responses API lane

`butler.agent.runtime.run_command` handles text, legacy voice, LLM-requesting
...
1. `butler/personality/monkey_butler.md`.
...
SQLite prompt memory is limited by `BUTLER_PROMPT_MEMORY_CHAR_LIMIT`, currently
...
The response ID is kept only in process memory. Restarting Butler removes that
...
voice worker
  -> POST /chat/stream
  -> butler.agent.runtime.run_command_stream
...
- Streaming enabled.
- Thinking mode: `never`.
- Maximum output: inherited from `BUTLER_AGENT_MAX_OUTPUT_TOKENS=1024`.
...
With the current `BUTLER_LLM_THINK_MODE=never` setting, the outgoing user
message receives:
...
After one or more function calls are executed, Butler sends:
...
The configured `butler-local` FastMCP server registers:
...
```text
BUTLER_SCOPE_TOOL_SURFACE=0
BUTLER_REDUCE_KICAD_TOOL_SURFACE=0
```
...
register it. The configured `BUTLER_MCP_SERVERS` value contains only the
`butler-local` aggregate, so TV tools are not part of the intended current
surface.

## MCP lifecycle and routing

`butler/mcp_client/client.py` supports:
...
`BUTLER_FILESYSTEM_ALLOW_WRITES` is enabled.
...
`BUTLER_CONTEXT_SNAPSHOT_CHAR_LIMIT` (4000), a backstop above the broker's
...
2. Project summary for the configured project, default `Butler`.
...
```text
Butler durable memory (compact, read-only):
```
...
asks Butler to remember a stable fact. The host implementation does not verify
