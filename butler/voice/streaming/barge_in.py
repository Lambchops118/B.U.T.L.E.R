...
1. :class:`BargeInDetector` runs on every captured microphone frame (~64 ms) and
   only while Butler is speaking. On a sustained rise above the expected echo
...
rest of :mod:`butler.voice.streaming`.
"""
...
from butler.voice.streaming.barge_in_observability import BargeInMetrics
...
class BargeInConfig:
    ...
    ``settings.env`` (see ``BUTLER_BARGE_IN_*``).
    """
...
def is_stop_command(text: str) -> bool:
    """True when the utterance only asks Butler to stop talking."""
