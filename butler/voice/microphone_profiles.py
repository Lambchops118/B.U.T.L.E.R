...
MICROPHONE_PROFILES = {
    "respeaker": MicrophoneProfile(
        ...
        capture_endpoint_env="BUTLER_RESPEAKER_CAPTURE_ENDPOINT_ID",
        device_name_env="BUTLER_RESPEAKER_DEVICE_NAME",
        ...
        # `python -m butler.voice.diagnostics.windows_aec_probe` and set this
    ...
    "yeti": MicrophoneProfile(
        name="yeti",
        label="Blue Yeti",
        capture_endpoint_env="BUTLER_YETI_CAPTURE_ENDPOINT_ID",
        device_name_env="BUTLER_YETI_DEVICE_NAME",
...
def resolve_energy_threshold(
    ...
    try:
        ...
        raise ValueError(
            "BUTLER_RECOGNIZER_ENERGY_THRESHOLD must be positive or 'auto'."
        ) from exc
    if threshold <= 0:
        raise ValueError(
            "BUTLER_RECOGNIZER_ENERGY_THRESHOLD must be positive or 'auto'."
