...
from butler.config import env_bool, env_float, env_int, load_environment, require_env
from butler.services import awareness_signals
from butler.telemetry import emit_pipeline_event
from butler.text.service_client import (
    send_interrupt,
    send_message,
    send_prewarm,
    stream_message,
)
from butler.voice.asr_queue import AsrPriority, BoundedAsrQueue
from butler.voice.benchmarking import VoiceBenchmarkSession
from butler.voice.microphone_profiles import (
    get_microphone_profile,
    resolve_energy_threshold,
)
from butler.voice.streaming import barge_in as barge_in_module
from butler.voice.streaming.barge_in import BargeInConfig, BargeInDetector, SpeechSession
from butler.voice.streaming.barge_in_observability import (
    FixtureRecorderConfig,
    SynchronizedFixtureRecorder,
)
from butler.voice.streaming.speaker import StreamingSpeaker
from butler.voice.streaming.vad import select_vad_lane
...
VOICE_SESSION_ID = os.getenv("BUTLER_VOICE_SESSION", "voice-worker")
VOICE_AGENT_URL = os.getenv("BUTLER_TEXT_AGENT_URL", "http://127.0.0.1:8420")
VOICE_AGENT_TOKEN = os.getenv("BUTLER_TEXT_AGENT_TOKEN", os.getenv("TEXT_AGENT_API_TOKEN", ""))
VOICE_AGENT_TIMEOUT = float(os.getenv("BUTLER_TEXT_AGENT_CLIENT_TIMEOUT", "30"))
VOICE_STREAMING = env_bool("BUTLER_VOICE_STREAMING", True)
...
LOCAL_STT_ENABLED = env_bool("BUTLER_LOCAL_STT", True)
...
REMOTE_STT_FALLBACK_ENABLED = env_bool("BUTLER_REMOTE_STT_FALLBACK", False)
...
# off the machine.
REMOTE_LLM_FALLBACK_ENABLED = env_bool("BUTLER_REMOTE_LLM_FALLBACK", False)
...
PRERAMP_ENABLED = env_bool("BUTLER_AGENT_PRERAMP", True)
...
BOOT_PHRASE_ENABLED = env_bool("BUTLER_BOOT_PHRASE", True)
BOOT_PHRASE = os.getenv("BUTLER_BOOT_PHRASE_TEXT", "").strip() or (
    'This is the initial boot phrase. Simply say: "Booting complete. Good Day Sir"'
)
BOOT_PHRASE_SESSION_ID = os.getenv("BUTLER_BOOT_PHRASE_SESSION", "voice-boot-warmup")

audio_interface = pyaudio.PyAudio()
aws_access_key = os.getenv("AWS_ACCESS_KEY")
aws_secret_key = os.getenv("AWS_SECRET_KEY")
VOICE_AUDIO_OUTPUT_DEVICE_INDEX = os.getenv("BUTLER_AUDIO_OUTPUT_DEVICE_INDEX")
...
POLLY_REGION = os.getenv("BUTLER_POLLY_REGION", "").strip() or "us-east-1"
...
POLLY_KEEPALIVE_SECONDS = float(os.getenv("BUTLER_POLLY_KEEPALIVE_SECONDS", "60"))
...
BARGE_IN_ENABLED = env_bool("BUTLER_BARGE_IN", False)
BARGE_IN_BACKEND = os.getenv("BUTLER_BARGE_IN_BACKEND", "aec").strip().lower()
...
BARGE_IN_REQUIRE_WAKE_WORD = env_bool("BUTLER_BARGE_IN_REQUIRE_WAKE_WORD", False)
# Session the proactive lane records its turns under (see butler/router.py); an
# interrupted alert has to be corrected where it was written.
PROACTIVE_SESSION_ID = os.getenv("BUTLER_VOICE_PROACTIVE_SESSION", "voice")
# Must stay above the agent's own BUTLER_INTERRUPTION_LOCK_TIMEOUT, so a slow
# correction is waited out rather than abandoned while the server still applies
# it behind our back.
VOICE_INTERRUPT_TIMEOUT = env_float("BUTLER_VOICE_INTERRUPT_TIMEOUT", 15.0)
...
# the reply speaks.
WAKE_CUE_ENABLED = env_bool("BUTLER_WAKE_CUE", True)
WAKE_CUE_DIR = Path(
    os.getenv("BUTLER_WAKE_CUE_DIR")
    or (Path(__file__).resolve().parent / "assets")
)

_BARGE_IN_CONFIG = BargeInConfig(
    floor_rms=env_int("BUTLER_BARGE_IN_FLOOR_RMS", 550),
    echo_margin=env_float("BUTLER_BARGE_IN_ECHO_MARGIN", 2.2),
    output_delay_ms=env_float("BUTLER_BARGE_IN_OUTPUT_DELAY_MS", 180.0),
    trigger_frames=env_int("BUTLER_BARGE_IN_TRIGGER_FRAMES", 3),
    endpoint_silence_ms=env_float("BUTLER_BARGE_IN_ENDPOINT_SILENCE_MS", 550.0),
    min_speech_ms=env_float("BUTLER_BARGE_IN_MIN_SPEECH_MS", 260.0),
    duck_gain=env_float("BUTLER_BARGE_IN_DUCK_GAIN", 0.12),
)
...
_asr_queue = BoundedAsrQueue(capacity=env_int("BUTLER_ASR_QUEUE_CAPACITY", 8))
...
IDLE_VAD_ENDPOINTING_REQUESTED = env_bool(
    "BUTLER_IDLE_VAD_ENDPOINTING",
    env_bool("BUTLER_VAD_ENDPOINTING", False),
)
IDLE_VAD_CORPUS_ACCEPTED = env_bool("BUTLER_IDLE_VAD_CORPUS_ACCEPTED", False)
...
_FIXTURE_RECORDING_ENABLED = env_bool(
    "BUTLER_BARGE_IN_FIXTURE_RECORDING", False
)
...
def _build_fixture_recorder() -> SynchronizedFixtureRecorder:
    config = FixtureRecorderConfig(
        ...
        max_duration_seconds=env_float(
            "BUTLER_BARGE_IN_FIXTURE_MAX_SECONDS", 120.0
        ),
        max_pcm_bytes=env_int(
            "BUTLER_BARGE_IN_FIXTURE_MAX_PCM_BYTES", 32 * 1024 * 1024
        ),
        max_sessions=env_int("BUTLER_BARGE_IN_FIXTURE_MAX_SESSIONS", 5),
        queue_frames=env_int("BUTLER_BARGE_IN_FIXTURE_QUEUE_FRAMES", 512),
    )
    try:
...
def _resolve_output_device_index():
    ...
    try:
        ...
        raise RuntimeError(
            "BUTLER_AUDIO_OUTPUT_DEVICE_INDEX must be an integer if set."
        ) from exc
...
def _build_profile_microphone():
    ...
    from butler.voice.streaming.portaudio_input import PortAudioChannelMicrophone
...
def _barge_in_available() -> bool:
    ...
    is mostly Butler's own voice -- it would cut itself off constantly. If local
...
class _MicrophoneTap:
    ...
    Two jobs. It feeds every frame to the detector while Butler is speaking, and
    ...
    blob. Muting it also drops the wasted transcription passes Butler currently
...
def _confirm_barge_in(
    ...
    """
    from butler.voice.backends.base import AudioChunk
    ...
    if (
        ...
        < env_float("BUTLER_BARGE_IN_ASR_MIN_DURATION_SECONDS", 0.16)
    ):
    ...
    if (
        ...
        < env_float("BUTLER_BARGE_IN_ASR_MIN_AVG_LOGPROB", -1.2)
    ):
    ...
    if (
        ...
        > env_float("BUTLER_BARGE_IN_ASR_MAX_NO_SPEECH_PROBABILITY", 0.6)
    ):
...
def _get_stt_backend():
    ...
    if _stt_backend is None:
        with _stt_backend_lock:
            if _stt_backend is None:
                from butler.voice.backends.factory import get_stt_backend
...
def _acquire_transcript(audio_data, benchmark):
    ...
    when ``BUTLER_REMOTE_STT_FALLBACK=1`` is explicitly configured.
    """
...
def run_speak_server(port: int | None = None):
    """Serve a loopback endpoint the rest of Butler uses to make the assistant
    ...
    if port is None:
        port = int(os.getenv("BUTLER_VOICE_SPEAK_PORT", "8610"))
    ...
    threading.Thread(
        target=server.serve_forever, name="butler-voice-speak", daemon=True
    ).start()
...
def _start_aec_duplex():
    ...
    from butler.voice.streaming.duplex import build_windows_duplex_pipeline
    from butler.voice.streaming.vad import (
        BargeInVadGate,
        SileroProbabilityVAD,
        VadGateConfig,
    )
    from butler.voice.streaming.windows_audio import (
    ...
    capture_id = (
        ...
        or os.getenv("BUTLER_AUDIO_CAPTURE_ENDPOINT_ID", "").strip()
    )
    render_id = os.getenv("BUTLER_AUDIO_RENDER_ENDPOINT_ID", "").strip()
    if not capture_id or not render_id:
        raise RuntimeError(
            "AEC barge-in requires pinned BUTLER_AUDIO_CAPTURE_ENDPOINT_ID and "
            "BUTLER_AUDIO_RENDER_ENDPOINT_ID values."
        )
    ...
    _barge_vad_gate = BargeInVadGate(
        ...
        config=VadGateConfig(
            start_probability=env_float("BUTLER_BARGE_IN_VAD_START", 0.65),
            end_probability=env_float("BUTLER_BARGE_IN_VAD_END", 0.35),
            start_frames=env_int("BUTLER_BARGE_IN_VAD_START_FRAMES", 3),
            end_silence_ms=env_float("BUTLER_BARGE_IN_ENDPOINT_SILENCE_MS", 480.0),
            min_speech_ms=env_float("BUTLER_BARGE_IN_MIN_SPEECH_MS", 160.0),
            preroll_ms=env_float("BUTLER_BARGE_IN_PREROLL_MS", 500.0),
            max_utterance_ms=env_float(
                "BUTLER_BARGE_IN_MAX_UTTERANCE_MS", 9000.0
            ),
        ),
    )
    if IDLE_VAD_ENDPOINTING_ENABLED:
        ...
        _idle_vad_gate = BargeInVadGate(
            ...
            config=VadGateConfig(
                start_probability=env_float("BUTLER_IDLE_VAD_START", 0.50),
                end_probability=env_float("BUTLER_IDLE_VAD_END", 0.35),
                start_frames=env_int("BUTLER_IDLE_VAD_START_FRAMES", 2),
                end_silence_ms=env_float(
                    "BUTLER_IDLE_VAD_ENDPOINT_SILENCE_MS", 480.0
                ),
                min_speech_ms=env_float("BUTLER_IDLE_VAD_MIN_SPEECH_MS", 160.0),
                preroll_ms=env_float("BUTLER_IDLE_VAD_PREROLL_MS", 640.0),
                max_utterance_ms=env_float(
                    "BUTLER_IDLE_VAD_MAX_UTTERANCE_MS",
                    env_float("BUTLER_VAD_MAX_UTTERANCE_MS", 15000.0),
                ),
            ),
        )
    else:
...
def run_voice_recognition():
    ...
    if IDLE_VAD_ENDPOINTING_REQUESTED and not IDLE_VAD_CORPUS_ACCEPTED:
        print(
            ...
            "BUTLER_IDLE_VAD_CORPUS_ACCEPTED=1. Using SpeechRecognition."
        )
    if not MICROPHONE_PROFILE.windows_aec:
        _barge_in_runtime_ready = False
        _vad_endpointing_active = False
        if BARGE_IN_ENABLED:
            print(
                ...
                "'python -m butler.voice.diagnostics.windows_aec_probe'."
            )
        ...
        try:
            ...
            from butler.voice.streaming.duplex import DuplexRecognizerAudioSource
...
def _handle_command_legacy(command, benchmark=None, session_id=None):
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(prefix="butler_speech_", suffix=".wav", delete=False) as tmp_file:
