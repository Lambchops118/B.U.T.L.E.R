...
from butler.config import env_bool, load_environment
from butler.memory import get_default_memory_store
from butler.phone.elevenlabs_twilio import ElevenLabsTwilioProvider
from butler.phone.provider import (
    OutboundCallRequest,
    PhoneConfig,
    PhoneProvider,
    is_e164_number,
...
from butler.phone.store import PhoneCallRecord, PhoneCallStore, get_default_phone_store
...
def place_phone_call(
    ...
    *,
    ...
    if not config.allowed_outbound:
        raise RuntimeError("Outbound phone calls are disabled. Set BUTLER_PHONE_ALLOWED_OUTBOUND=1 to enable them.")
...
def _build_provider(
    *,
    ...
    if resolved_config.provider_name != "elevenlabs_twilio":
        raise RuntimeError(
            f"Unsupported BUTLER_PHONE_PROVIDER '{resolved_config.provider_name}'. "
...
def _ensure_phone_enabled(config: PhoneConfig) -> None:
    if not config.enabled:
        raise RuntimeError("Phone support is disabled. Set BUTLER_PHONE_ENABLED=1 to enable it.")
...
def _resolve_target(contact_or_number: str, config: PhoneConfig) -> tuple[str, str | None]:
    ...
    if is_e164_number(raw_value):
        if raw_value not in set(config.allowlist):
            raise RuntimeError("That number is not in BUTLER_PHONE_ALLOWLIST.")
        return raw_value, None

    contact_key = normalize_contact_key(raw_value)
    if contact_key not in config.contacts:
        raise RuntimeError(
            "Unknown phone contact. Use a configured contact name or an E.164 number in BUTLER_PHONE_ALLOWLIST."
        )
...
def _maybe_record_call_memory(record: PhoneCallRecord) -> None:
    if not env_bool("BUTLER_MEMORY_ENABLED", True):
        return
    ...
    try:
        ...
        print(f"Butler phone memory write failed: {exc}")
...
def _compose_outbound_call_brief(
    *,
    ...
    lines = [
        ...
        "When the recipient answers, immediately identify yourself as Butler.",
...
_TRANSCRIPT_ROLE_LABELS = {"user": "Caller", "assistant": "Butler", "agent": "Butler"}
