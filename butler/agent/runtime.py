...
from butler.agent.thinking import thinking_suffix
from butler.config import env_bool, env_float, env_int, load_environment, require_env
from butler.memory import MemoryStore, get_default_memory_store
from butler.llm_debug import emit_llm_io
from butler.mcp_client import get_local_mcp_client, shutdown_local_mcp_client
from butler.telemetry import emit_pipeline_event
from butler.tool_arguments import parse_tool_arguments
...
from butler.voice.streaming.barge_in import truncated_transcript
...
ROUTER_MODEL = os.getenv("BUTLER_ROUTER_MODEL", ai_model)
MAX_TOOL_CALL_ROUNDS = max(1, env_int("BUTLER_MAX_TOOL_CALL_ROUNDS", 8))
MEMORY_ENABLED = env_bool("BUTLER_MEMORY_ENABLED", True)
...
TOOLS_DISABLED = env_bool("BUTLER_DISABLE_ALL_TOOLS", False)
...
STARTUP_WARMUP_ENABLED = env_bool("BUTLER_AGENT_STARTUP_WARMUP", True)
STARTUP_WARMUP_COMMAND = (
    os.getenv("BUTLER_AGENT_STARTUP_WARMUP_COMMAND", "").strip()
...
PRERAMP_MIN_INTERVAL_SECONDS = max(
    0.0, env_float("BUTLER_AGENT_PRERAMP_MIN_INTERVAL", 5.0)
)
PROMPT_MEMORY_CHAR_LIMIT = max(0, env_int("BUTLER_PROMPT_MEMORY_CHAR_LIMIT", 1600))
CONVERSATION_HISTORY_MESSAGE_LIMIT = max(
    0,
    env_int("BUTLER_CONVERSATION_HISTORY_MESSAGE_LIMIT", 8),
)
CONVERSATION_HISTORY_CHAR_LIMIT = max(
    0,
    env_int("BUTLER_CONVERSATION_HISTORY_CHAR_LIMIT", 4000),
)
...
INTERRUPTION_LOCK_TIMEOUT_SECONDS = max(
    0.1,
    env_float("BUTLER_INTERRUPTION_LOCK_TIMEOUT", 10.0),
)
OPENAI_SERVER_ERROR_RETRIES = max(0, env_int("BUTLER_OPENAI_SERVER_ERROR_RETRIES", 2))
OPENAI_SERVER_ERROR_RETRY_DELAY = max(
    0.0, env_float("BUTLER_OPENAI_SERVER_ERROR_RETRY_DELAY", 1.5)
)
OPENAI_SERVER_ERROR_RETRY_MAX_DELAY = max(
    OPENAI_SERVER_ERROR_RETRY_DELAY,
    env_float("BUTLER_OPENAI_SERVER_ERROR_RETRY_MAX_DELAY", 20.0),
)
OPENAI_SERVER_ERROR_RETRY_JITTER = max(
    0.0, env_float("BUTLER_OPENAI_SERVER_ERROR_RETRY_JITTER", 0.25)
)
OPENAI_SERVER_ERROR_RECOVERY_ATTEMPTS = max(
    0, env_int("BUTLER_OPENAI_SERVER_ERROR_RECOVERY_ATTEMPTS", 1)
)
AGENT_MAX_OUTPUT_TOKENS = max(150, env_int("BUTLER_AGENT_MAX_OUTPUT_TOKENS", 400))
...
INJECT_CURRENT_TIME = env_bool("BUTLER_INJECT_CURRENT_TIME", True)
# Counter the strongest in-context pattern the assistant ever sees: its own
# transcript. See :func:`_history_grounding_notice`.
INJECT_HISTORY_GROUNDING = env_bool("BUTLER_INJECT_HISTORY_GROUNDING", True)
...
CONTEXT_SNAPSHOT_CHAR_LIMIT = env_int("BUTLER_CONTEXT_SNAPSHOT_CHAR_LIMIT", 4000)
...
RECOVER_LEAKED_TOOL_CALLS = env_bool("BUTLER_RECOVER_LEAKED_TOOL_CALLS", True)
...
TOOL_OUTPUT_CHAR_LIMIT = max(256, env_int("BUTLER_TOOL_OUTPUT_CHAR_LIMIT", 4000))
TOOL_OUTPUT_SUMMARY_ENABLED = env_bool("BUTLER_SUMMARIZE_TOOL_OUTPUTS", True)
TOOL_OUTPUT_SUMMARY_PREVIEW_ITEMS = max(
    1, env_int("BUTLER_TOOL_OUTPUT_SUMMARY_PREVIEW_ITEMS", 5)
)
TOOL_OUTPUT_SUMMARY_PREVIEW_KEYS = max(
    1, env_int("BUTLER_TOOL_OUTPUT_SUMMARY_PREVIEW_KEYS", 12)
)
...
def _resource_tool_definitions() -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "name": "memory_admin",
            "description": (
                "Durable Butler memory. Use 'remember' when the user explicitly asks you to "
        ...
        },
        {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "message_to_deliver": {
                        "type": "string",
                        "description": "The exact report the phone agent should deliver after introducing itself as Butler.",
                    },
                    "call_id": {
                        "type": "string",
                        "description": "Butler phone call id, usually the ElevenLabs conversation id. Required by call_status and summarize_call.",
                    },
                    "limit": {
                        "type": "integer",
...
def _build_tool_definitions(
    ...
    if TOOLS_DISABLED:
        ...
        print("Tool surface disabled (BUTLER_DISABLE_ALL_TOOLS): sending 0 tools.")
        return []

    tool_defs = _resource_tool_definitions() + mcp_client.openai_tool_definitions()
    reduce_kicad = os.getenv("BUTLER_REDUCE_KICAD_TOOL_SURFACE", "1").strip().lower()
...
def note_interruption(
    ...
    *,
    ...
    try:
        ...
        print(f"Butler interruption record failed: {_truncate_text(str(exc), 300)}")
        return False
    finally:
        if acquired:
            lock.release()
...
def _get_memory_store() -> MemoryStore | None:
    ...
    try:
        ...
        print(f"Butler memory unavailable: {_truncate_text(str(exc), 300)}")
        return None
...
def _get_prompt_memory(
    ...
    *,
    ...
    try:
        ...
        print(f"Butler memory retrieval failed: {_truncate_text(str(exc), 300)}")
        return None
...
def _get_conversation_history(
    ...
    try:
        ...
        print(f"Butler conversation retrieval failed: {_truncate_text(str(exc), 300)}")
        return []
...
def _current_time_context() -> str | None:
    ...
    if not INJECT_CURRENT_TIME:
        return None
    try:
        from butler.services.home_automation import get_current_datetime

        now_block = get_current_datetime()
    except Exception as exc:
        print(f"Butler current-time context unavailable: {_truncate_text(str(exc), 200)}")
        return None
    return (
...
def classify_request_route(
    ...
    *,
    ...
    instructions = (
        "You are Butler's request router. Decide how the next user turn should be handled. "
        "Return only compact JSON with keys mode, reason, and response. Valid modes: foreground, background, status. "
        "Use foreground for ordinary chat, definitions, explanations, and questions about Butler behavior. "
        "Use background only when the user is asking Butler to perform long-running work that can continue after an acknowledgement. "
...
def _invoke_host_tool(
    ...
    *,
    ...
    if name == "remember_memory_fact":
        memory_store = _get_memory_store()
        if memory_store is None:
            return json.dumps({"success": False, "message": "Butler memory is disabled or unavailable."})
    ...
    if name == "place_phone_call":
        from butler.phone import place_phone_call
    ...
    if name == "phone_call_status":
        from butler.phone import phone_call_status
    ...
    if name == "recent_phone_calls":
        from butler.phone import recent_phone_calls
    ...
    if name == "summarize_phone_call":
        from butler.phone import summarize_phone_call
    ...
    *,
    ...
    if memory_store is not None:
        try:
            ...
            print(f"Butler memory session write failed: {_truncate_text(str(exc), 300)}")
            memory_store = None
...
def _get_stream_backend():
    ...
    with _stream_backend_lock:
        if _stream_backend is None:
            from butler.voice.backends.factory import get_llm_backend
...
def reset_session(session_id: str) -> None:
    ...
    try:
        ...
        print(f"Butler memory session reset failed: {_truncate_text(str(exc), 300)}")
